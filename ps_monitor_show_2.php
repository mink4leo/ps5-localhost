...
<head>
    ...
    <style>
        ...
        .dd {
            ...
            background: rgb(59, 50, 230);
            background: linear-gradient(90deg, rgba(59, 50, 230, 1) 0%, rgba(135, 8, 191, 1) 53%, rgba(226, 14, 202, 1) 100%);
            /* color: #; */
        }
    </style>
</head>
...
<body>
    ...
    <?php if ($res['idstart'] == 0) { ?>
        <?php include 'ps_table_show.php' ?>
    <?php } elseif ($res['idstart'] <> 0 && $res['restartA'] == '0') { ?>
        <div class="step2">
            <?php
            // Refresh +++++++
            $updateR = $db->prepare("UPDATE $dataStart set restartA = '1'");
            $updateR->execute();
            //======================
            $player_select = $db->prepare("SELECT * FROM thannam_champion_poomsae WHERE id = '$res[idstart]'");
            $player_select->execute();
            $player = $player_select->fetch();
            // TEAM ================
            $team_select = $db->prepare("SELECT * FROM thannam_champion_teamm WHERE id = '$player[idteam]'");
            $team_select->execute();
            $team = $team_select->fetch();
            // CAT ================
            $cat_select = $db->prepare("SELECT * FROM thannam_champion_groupteam WHERE id = '$player[catA]'");
            $cat_select->execute();
            $gpw = $cat_select->fetch();


            if ($player['ps_cat'] == 2) {
                $ps_poomsae = "INDIVIDUAL";
            } elseif ($player['ps_cat'] == 3) {
                $ps_poomsae = "PAIR";
            } elseif ($player['ps_cat'] == 4) {
                $ps_poomsae = "TEAM";
            } elseif ($player['ps_cat'] == 8) {
                $ps_poomsae = "FREE STYLE";
            }
            ?>
            <div class="aa">POOMSAE <?php echo $ps_poomsae; ?></div>
            <div class="dd"><?php echo $gpw['namegroup'] ?></div>
            <div class="bb"><?php echo $player['nameA'] ?></div>
            <div class="cc"><?php echo $team['nameA'] ?></div>
        </div>
    <?php } else { ?>
        <div class="mt_5">
            <table class="table">
                <tr>
                    <td>
                        <div class="flex">
                            <div>
                                <img src="img/fast-forward.png" class="width-200">
                            </div>
                            <div>
                                <div class="head2">
                                    <?php if ($ps == 1) { ?>
